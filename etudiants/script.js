...
function renderNotes(n){
  ...
  /* Calcul auto Moyenne Générale Bac :
     50% National + 25% CC 2e année + 25% Régional */
  var nat=parseFloat(n.moy_nationale);
  var cc2=parseFloat(n.moy_an2);
  var reg=parseFloat(n.moy_regional);
  if(!isNaN(nat)&&!isNaN(cc2)&&!isNaN(reg)){
    n.moy_gen_bac=Math.round((0.5*nat+0.25*cc2+0.25*reg)*100)/100;
  }

  ['an1_s1','an1_s2','moy_an1','moy_regional',
   'an2_s1','an2_s2','moy_an2',
   'moy_nationale','moy_gen_bac'].forEach(function(id){
...
function renderTable(cands){
  ...
  tbody.innerHTML=cands.map(function(c,i){
    ...
    var recuDl=c.recu_lien?'<a class="recu-btn" href="'+c.recu_lien+'" target="_blank" rel="noopener">📄 PDF</a>':'<span class="cell-na">—</span>';
...
function renderCards(cands){
  ...
  grid.innerHTML=cands.map(function(c,i){
    ...
    var recuBtn=c.recu_lien?'<a class="card-btn-recu" href="'+c.recu_lien+'" target="_blank" rel="noopener">📄 Voir le reçu</a>':'';
