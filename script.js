...
var calEvents = {};   /* 'YYYY-MM-DD' -> [abrev, ...] */
...
function initCalendar(data){
  ...
  for(var i=0;i<data.length;i++){
    ...
    for(var j=0;j<dates.length;j++){
      ...
      var key = parts[0];
      var label = parts[1] ? e.abrev + ' (' + parts[1] + ')' : e.abrev;
      if(!calEvents[key]) calEvents[key] = [];
      if(calEvents[key].indexOf(label) === -1) calEvents[key].push(label);
    }
  }
  var now = new Date();
...
function renderCalendar(){
  ...
  for(var d=1;d<=daysInMonth;d++){
    ...
    for(var k=0;k<evs.length;k++){
      cell += '<div class="cal-badge" title="'+evs[k]+'">'+evs[k]+'</div>';
    }
    cell += '</div>';
    html += cell;
  }
